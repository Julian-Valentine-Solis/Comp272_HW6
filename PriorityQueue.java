...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        ...
        // YOUR CODE GOES HERE
        // Use constructor of node to build a node with given parameters and use
        // tree.size() to place at end of array
        Node node = new Node(e, priority, tree.size());

        // Add the node into our min-heap
        tree.add(node);

        // Utilize to the pre-coded pullUp method to percolate up the node plac ed to
        // restore min-heap property
        pullUp(node.idx);

        // Return the placed node
        return node;
    }
    ...
    public boolean contains(E e) {
        ...
        // ADD YOUR CODE HERE
        // We have an element, and we need to
        // Use a loop to iterate
        // for each node in the min-heap
        for (Node node : tree) {
            if (node.value.equals(e)) {// if a node's value in the heap matches the element given, then it is present
                                       // in the minheap
                return true;// return true, it was found
            }
        }
        return false;// return false, it was never found
    }
